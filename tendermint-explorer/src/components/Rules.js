...
class Rules extends Component {
    ...
    buildFromRules(rules) {
        ...
        let reduceToComponents =
            _.bind(
                (settings) => _.reduce(settings,
                    // Function
                    ([result, acc, self], s) => {
                    ...
                    [[], 1, this]), this)
        let result = _.map(settings, (s)  => { let result, ev; [result, ...ev] = reduceToComponents(s); return result})
        return result;
    }
    maxTemp(key, value) { return (
