...
func (app *PersistentKeyValueApplication) Info(req types.RequestInfo) types.ResponseInfo {
	...
	latestVersion, err := app.app.state.Load()
	if err != nil {
		panic(err)
	}
...
func (app *PersistentKeyValueApplication) Commit() types.ResponseCommit {
	...
	appHash, newVersion, err = app.app.state.SaveVersion()
	if err != nil {
		panic(err)
	}

	app.logger.Info("Commit block", "height", newVersion, "root", appHash)
...
func (app *PersistentKeyValueApplication) execSensorTx(tx []byte) types.ResponseDeliverTx {
	...
	tags := []cmn.KVPair{
		{Key: []byte("app.sensor"), Value: []byte(key)},
		{Key: []byte("app.value"), Value: []byte(value)},
		{Key: []byte("app.timestamp"), Value: []byte(timestamp)},
	}
